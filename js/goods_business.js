...
function initTableData() {
	...
	function check() {
		...
		if(row.length == 0) {
			...
			if(status == 1) {
				...
				$.ajax({
					type: "get",
					url: "" + row[0].id + "&operator=" + operator,
					xhrFields: {
						withCredentials: true
					},
					success: function(data) {
		...
		$.ajax({
			...
			success: function(data) {
				if(data.code == 200) {
					...
					$("#customerShow").append('<span id="a3">' + data.data.all.supplier + '</span>');
	...
	function searchBtn() {
		var searchNumber = $("#searchCustomer").val();
	...
	function initCombox(it) {
		$(it).combobox({
			method: "get",
			url: "",
			textField: "to_customer",
			valueField: "to_customer"
